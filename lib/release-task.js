...
class ReleaseTask {
  ...
  */
  async execute() {
    try {
      await this.persistStatus(RELEASE_TASK_STATUSES.ONGOING);
      await this.release();
  ...
  const result = await query(`
    ...
    SELECT ?s WHERE {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        ?s a ext:ReleaseTask ;
         dct:created ?created ;
         adms:status <${RELEASE_TASK_STATUSES.ONGOING}> .
      }
...
async function getNextReleaseTask() {
  const result = await query(`
    ...
    SELECT ?s ?source ?created WHERE {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        ?s a ext:ReleaseTask ;
          adms:status <${RELEASE_TASK_STATUSES.READY}> ;
          dct:source ?source ;
          dct:created ?created .
  ...
  if (result.results.bindings.length) {
    ...
    return new ReleaseTask({
      ...
      status: RELEASE_TASK_STATUSES.READY,
...
function logFailureResolutionManual(failedTask) {
  ...
  console.log(`
    ...
    DELETE {
      ...
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${failedTask}> adms:status <${RELEASE_TASK_STATUSES.READY}> .
      }
    } WHERE {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${failedTask}> adms:status ?status .
