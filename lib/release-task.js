...
class ReleaseTask {
  ...
  */
  async release() {
    ...
    console.log(`Linking dataset <${dataset.datasetUri}> to task <${this.uri}> ...`);
    await linkTaskToDataset(this.uri, dataset.datasetUri);

    console.log(`Deprecating the previous dataset of dataset <${dataset.datasetUri}> ...`);
    await dataset.deprecatePrevious();

    console.log(`Releasing the new dataset <${dataset.datasetUri}> ...`);
    await dataset.release();

    console.log(`Updating catalog <${BESLUITVORMING_CATALOG_URI}>...`);
    await updateCatalog(dataset.datasetUri);
  };
 ...
 * @public
*/
async function linkTaskToDataset(taskUri, datasetUri) {
  await update(`
    PREFIX prov: <http://www.w3.org/ns/prov#>

    INSERT DATA {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${taskUri}> prov:generated <${datasetUri}> .
      }
    }
  `);
}
