...
class Dataset {
  ...
  */
  async generateTtlFile() {
    ...
    writeToFile(this.filePath, this.graph);
  }
      ...
      INSERT {
        GRAPH <${this.graph}> {
          ...
          GRAPH <${this.graph}> {
            ...
            OPTIONAL { ?s mu:thisDoesntExists ?null }
            BIND (IF(BOUND(?null), ?null, STRUUID()) as ?distributionUuid)
            BIND (IRI(CONCAT("${uri}", ?distributionUuid)) AS ?uri)
            BIND (IRI(CONCAT("${HOST_DOMAIN}/files/", ?logicalFileUuid, "/download" )) as ?downloadURL)
          }
      }
  `);
  }
  ...
  async deprecatePrevious() {
    ...
    if (result.results.bindings.length) {
      ...
      await update(`
        ...
        INSERT DATA {
          GRAPH <${this.graph}> {
            <${this.uri}> prov:revisionOf <${previousDataset}> .
          }
        }
      `);
