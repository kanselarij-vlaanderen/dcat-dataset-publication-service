...
class Dataset {
   ...
   */
  async release() {
    await this.preRelease();
    console.log(`Making data from the dataset available in the public graph (<${PUBLIC_GRAPH}>)`);
    console.log('waiting before moving data to force timing issue')
    await this.sleep(80000);
    await moveGraph(this.graph, PUBLIC_GRAPH);
    console.log('waiting after moving data to force timing issue')
    await this.sleep(80000);
    await this.releaseDataset();
  ...
  /**
   * Link the uri of new dataset to the meeting
   *
   * @private
   */
    async preRelease() {
      ...
      console.log(`Dataset <${this.uri}> linked to the meeting in temp graph`);
    }
   ...
   * @private
   */
  async releaseDataset() {
    const now = Date.now();

    // delete/insert as workaroun for cache issue
    await update(`
    ...
    console.log(`Dataset <${this.uri}> released. It can be consumed now.`);
  }
  ...
  */
  ...
  async deprecatePrevious() {
    ...
    if (result.results.bindings.length) {
      ...
      console.log(`No previous dataset found to deprecate`);
    }
  }
